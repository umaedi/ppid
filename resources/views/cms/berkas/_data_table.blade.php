    ...
    <tbody>
        @forelse ($table as $key =>  $tb)
            <tr>
                ...
                <td>{{ $tb->created_at !== null ? $tb->created_at->format('d-m-Y') : 'Tidak ditemukan' }}</td>
                <td class="d-flex gap-1">
                    <a href="{{ route('dokumen', ['filename' => $tb->berkas ?? '-']) }}" class="badge bg-primary"><i
                            class='bx bx-download'></i></a>
                    <button class="badge bg-info border-0" onclick="salinTautan('{{ $tb->berkas }}')"><i
                            class='bx bx-copy'></i></button>
                    {{-- <button class="badge bg-warning border-0"
                        onclick="updateBerkas('{{ $tb->id }}', '{{ $tb->nama_berkas }}')"><i
                            class='bx bxs-edit'></i></button> --}}
                    <button class="badge bg-danger border-0" onclick="hapusBerkas('{{ $tb->id }}')"><i
                                ...
                                <div class="empty-action">
                                    <button onclick="loadData()" class="btn btn-primary">
                                        <svg xmlns="http://www.w3.org/2000/svg"
                                            class="icon icon-tabler icon-tabler-refresh" width="18" height="18"
                                            viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                            stroke-linecap="round" stroke-linejoin="round">
                                            <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
